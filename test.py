...
#    is_real = is_real_image[level]
#    return render_template('game_page.html', image=image, headline=headline, real=is_real)
...
def check_answer(is_real):
    user_guess = request.form.get['action']
    if request.method == 'POST':
        if (user_guess == "ai" and is_real == False) or (user_guess == "real" and is_real == True):
            score += 1
            return "Correct"
        else:
            score -= 1 # and tell player that they're wrong
            return "WRONG!"
    else:
        return "Error"
